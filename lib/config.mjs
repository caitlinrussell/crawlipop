...
function normalizeGoogleCredentials(credentials) {
  ...
  return {
    ...credentials,
    client_email:
      typeof credentials.client_email === "string" ? credentials.client_email.trim() : credentials.client_email,
    private_key:
...
async function loadGoogleCredentials() {
  ...
  if (rawJson) {
    return normalizeGoogleCredentials(JSON.parse(rawJson));
  }
  ...
  return normalizeGoogleCredentials(await readJsonFile(resolvedPath));
}
