...
function hasValidation(row) {
  return Object.prototype.hasOwnProperty.call(row, "validation");
}

function currentMetrics(row) {
  return hasValidation(row) ? row.validation : row;
}

function stillCurrent(row, predicate) {
  const metrics = currentMetrics(row);
  return Boolean(metrics) && predicate(metrics);
}

function validationEvidence(row) {
  const metrics = row.validation;
  if (!metrics?.startDate || !metrics?.endDate) {
    return [];
  }

  return [
    `Latest check ${metrics.startDate} to ${metrics.endDate}: ${compactNumber(metrics.impressions)} impressions, ${formatPercent(metrics.ctr)} CTR, position ${metrics.position.toFixed(1)}`
  ];
}
...
function buildCtrSuggestions(topQueries) {
  return topQueries
    .filter((row) =>
      ...
      row.position <= 10 &&
      row.ctr <= 0.03 &&
      stillCurrent(row, (metrics) => metrics.impressions >= 75 && metrics.position <= 10 && metrics.ctr <= 0.03)
    )
    .map((row) => ({
...
function buildPositionSuggestions(topQueries) {
  return topQueries
    .filter((row) =>
      ...
      row.position >= 5 &&
      row.position <= 15 &&
      stillCurrent(row, (metrics) => metrics.impressions >= 50 && metrics.position >= 5 && metrics.position <= 15)
    )
    .map((row) => ({
...
function buildRefreshSuggestions(topPages) {
  return topPages
    .filter((row) =>
      row.clicksChange !== null &&
      row.clicksChange <= -0.15 &&
      row.position <= 12 &&
      stillCurrent(
        row,
        (metrics) => metrics.clicksChange !== null && metrics.clicksChange <= -0.1 && metrics.position <= 12
      )
    )
    .map((row) => ({
...
function buildPageCtrSuggestions(topPages) {
  return topPages
    .filter((row) =>
      ...
      row.position <= 12 &&
      row.ctr <= 0.03 &&
      stillCurrent(row, (metrics) => metrics.impressions >= 5 && metrics.position <= 12 && metrics.ctr <= 0.03)
    )
    .map((row) => ({
...
function buildEmergingPageSuggestions(topPages) {
  return topPages
    .filter((row) =>
      ...
      row.impressions >= 10 &&
      row.position <= 8 &&
      stillCurrent(row, (metrics) => metrics.clicks === 0 && metrics.impressions >= 3 && metrics.position <= 8)
    )
    .map((row) => ({
...
function buildMomentumSuggestions(topPages) {
  return topPages
    .filter((row) =>
      row.clicksChange !== null &&
      row.clicksChange >= 0.2 &&
      row.position >= 6 &&
      row.position <= 15 &&
      stillCurrent(
        row,
        (metrics) => metrics.clicksChange !== null && metrics.clicksChange >= 0.1 && metrics.position >= 6 && metrics.position <= 15
      )
    )
    .map((row) => ({
