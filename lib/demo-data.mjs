...
function createDemoBehaviorAnalysis(anchorDate, windowDays = 14) {
  const safeWindowDays = [7, 14, 28, 60, 90].includes(Number.parseInt(windowDays, 10))
    ? Number.parseInt(windowDays, 10)
    : 14;
  const windowStart = `${daysAgo(anchorDate, safeWindowDays - 1)}T00:00:00.000Z`;
...
export function createDemoDashboard(siteUrl, windowDays = 28) {
  const safeWindowDays = [7, 14, 28, 60, 90].includes(Number.parseInt(windowDays, 10))
    ? Number.parseInt(windowDays, 10)
    : 28;
  ...
  return {
    ...
    dateWindow: {
      recent: {
        startDate: daysAgo(dataAnchor, safeWindowDays - 1),
        endDate: dataAnchor
      },
      previous: {
        startDate: daysAgo(dataAnchor, safeWindowDays * 2 - 1),
        endDate: daysAgo(dataAnchor, safeWindowDays)
      }
    },
    validationWindow: {
      recent: {
        startDate: daysAgo(dataAnchor, Math.min(7, safeWindowDays) - 1),
        endDate: dataAnchor
      },
      previous: {
        startDate: daysAgo(dataAnchor, Math.min(7, safeWindowDays) * 2 - 1),
        endDate: daysAgo(dataAnchor, Math.min(7, safeWindowDays))
      }
    },
    connection: {
      searchConsole: {
    ...
    behaviorAnalysis: createDemoBehaviorAnalysis(dataAnchor, safeWindowDays),
    ticketsBySuggestion: {}
  };
}
